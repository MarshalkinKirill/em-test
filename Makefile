...
lint:
	@golangci-lint run --timeout=10m -c .golangci.yml
...
MAPPING := "classifier:yellotsrv.classTemplates,yellotsrv.inboxMessageClasses;extended:mailsrv.inboxMessages"

mfd-xml:
	@mfd-generator xml -c "postgres://postgres:x@localhost:6543/yellotsrvtest?sslmode=disable" -m ./docs/model/yellotsrv.mfd -t "yellotsrv.*,mailsrv.*" -n $(MAPPING)
mfd-model:
	@mfd-generator model -m ./docs/model/yellotsrv.mfd -p db -o ./pkg/db
mfd-repo: --check-ns
	@mfd-generator repo -m ./docs/model/yellotsrv.mfd -p db -o ./pkg/db -n $(NS)
mfd-vt-xml:
	@mfd-generator xml-vt -m ./docs/model/yellotsrv.mfd
mfd-vt-rpc: --check-ns
	@mfd-generator vt -m docs/model/yellotsrv.mfd -o pkg/vt -p vt -x yellotsrv/pkg/db -n $(NS)
mfd-xml-lang:
	#TODO: add namespaces support for xml-lang command
	@mfd-generator xml-lang  -m ./docs/model/yellotsrv.mfd
mfd-vt-template: --check-ns type-script-client
	@mfd-generator template -m docs/model/yellotsrv.mfd  -o ../gold-vt/ -n $(NS)
